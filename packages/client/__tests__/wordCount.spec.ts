...
describe('Words test', () => {
  ...
  it('Should pick chinese words correctly', () => {
    const chineseWords =
      getChinese(
        'raline - 一款从 Valine 衍生的带后端评论系统。可以将 raline 等价成 With backend Valine.',
      ) ?? [];
  ...
  it('Should omit other characters in Markdown', () => {
    ...
    expect(
      getWordNumber(
        'raline - 一款从 **Valine** 衍生的带后端评论系统。\n\n可以将 raline 等价成 _With backend Valine_.',
      ),
    ).toEqual(25);
  });

  it('Additional counts with Markdown links and images', () => {
    const linkAddress = '//unpkg.com/@raline/client/dist/raline.min.js';
    const linkMarkdown = `You can found raline [here](${linkAddress}).`;
...
<head>
  ...
  <link
    rel="stylesheet"
    href="https://unpkg.com/@raline/client@v3/dist/raline.css"
  />
  <!-- ... -->
</head>
<body>
  ...
  <div id="raline"></div>
  <script type="module">
    import { init } from 'https://unpkg.com/@raline/client@v3/dist/raline.js';
