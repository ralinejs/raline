...
import { getArticleCounter, updateArticleCounter } from '@raline/api';
import type { ComputedRef } from 'vue';
import { computed, inject, onMounted, onUnmounted, ref, watch } from 'vue';

import { LoadingIcon } from './Icons.js';
import { useReactionStorage } from '../composables/index.js';
import type { ralineReactionLocale } from '../typings/index.js';
import type { ralineConfig } from '../utils/index.js';
...
const reactionsInfo = computed<ReactionItem[]>(() => {
  ...
  return reaction.map((icon, index) => ({
    icon,
    desc: locale.value[`reaction${index}` as keyof ralineReactionLocale],
