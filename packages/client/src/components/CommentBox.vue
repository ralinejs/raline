...
const emoji = ref<DeepReadonly<ralineEmojiConfig>>({ tabs: [], map: {} });
...
const submitComment = async (): Promise<void> => {
  ...
  if (!props.edit) {
    // https://github.com/ralinejs/raline/issues/2163
  ...
  try {
    ...
    // ensure changes are applied to dom to avoid  https://github.com/ralinejs/raline/issues/2371
    await nextTick();
...
const onLogout = (): void => {
  userInfo.value = {};
  localStorage.setItem('raline_USER', 'null');
  sessionStorage.setItem('raline_USER', 'null');
  emit('log');
};
...
const onImageWallScroll = async (event: Event): Promise<void> => {
  ...
  const searchOptions = config.value.search as ralineSearchOptions;
...
useEventListener(
  ...
  ({ data }: { data: { type: 'profile'; data: UserInfo } }) => {
    ...
    [localStorage, sessionStorage]
      .filter((store) => store.getItem('raline_USER'))
      .forEach((store) =>
        store.setItem('raline_USER', JSON.stringify(userInfo)),
      );
  },
);
