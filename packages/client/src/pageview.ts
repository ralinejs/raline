import type { GetArticleCounterResponse } from '@raline/api';
import { getPageview, updatePageview } from '@raline/api';

import type { ralineAbort } from './typings/index.js';
import { errorHandler, getQuery, getServerURL } from './utils/index.js';

export interface ralinePageviewCountOptions {
  /**
   * raline 服务端地址
   *
   * raline server url
...
export const pageviewCount = ({
  ...
  selector = '.raline-pageview-count',
  update = true,
  lang = navigator.language,
}: ralinePageviewCountOptions): ralineAbort => {
